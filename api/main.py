...
class CustomUnpickler(pickle.Unpickler):
    def find_class(self,module,name):
        if name == 'CSWinTransformer':
            from cswin import CSWinTransformer
            return CSWinTransformer
        if name == 'CSWinBlock':
            from cswin import CSWinBlock
            return CSWinBlock
        if name == 'LePEAttention':
            from cswin import LePEAttention
            return LePEAttention
        if name == 'Merge_Block':
            from cswin import Merge_Block
            return Merge_Block
        if name == 'Mlp':
...
#model = torch.load(r"Cswin8291.pt",map_location=torch.device('cpu'))
...
def predict():
    ...
    #s = "Pred: " + class_names[target_image_pred_label] + "Prob: " + target_image_pred_probs.max()
    return [class_names[target_image_pred_label],round(target_image_pred_probs.max().tolist()*100,3)]
